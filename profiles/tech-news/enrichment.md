...
- `impact`: In 1-2 complete sentences, explain concrete, evidence-supported consequences for specifically affected users, developers, organizations, ecosystems, or standards. State the mechanism and distinguish observed effects from likely implications or uncertainty. Use `web_search` only when external evidence is necessary. Omit the block when it would merely repeat the summary or offer generic speculation.
