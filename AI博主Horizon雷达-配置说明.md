...
已准备：`data/config.ai-blogger.json`、`profiles/ai-creator/` 以及 GitHub Actions 工作流模板 `.github/workflows/ai-blogger-radar.yml.disabled`。
...
3. **启用工作流**：在你的 Fork 中，将 `.github/workflows/ai-blogger-radar.yml.disabled` 重命名为 `.github/workflows/ai-blogger-radar.yml`，提交后在 Actions 页面手动运行一次。成功后，在 Settings → Pages 中将 Source 设为 `Deploy from a branch`，选择 `gh-pages` / `root`。
...
> 模型供应商与密钥尚未确认，因此没有启用或提交任何自动化工作流，也没有创建 `.env` 文件。
